...
lemma p_edge_card {C : finset β} {p : ℕ} {edge: finset β } (h: edge ∈ p_edge C p) : edge.card = p:=
begin
...
lemma p_edge_in_ss_subset (C : finset β ) (ss : finset β) (p : ℕ):
...
begin
  -- haveI : linear_order α := is_well_order.linear_order (well_ordering_rel),
